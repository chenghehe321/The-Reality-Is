...
//				storage -= 2000;// 外地房租
			...
			if (storage + invest > 200000 && carFlag) {
				// System.out.println("买车：手里有[" + storage + "], 投资账户有[" + invest
				// + "]");
				carDay = i;
				storage += invest;
				invest = 0;
			...
			if (storage + invest > 200000 && houseFlag) {
//				storage += 1100000;
				// System.out.println("买房日:["+i+"], 手里有[" + storage +
				// "], 投资账户有[" + invest
				// + "]");
				storage += invest;
				invest = 0;
			...
			if (!carFlag && i - carDay > 365 * 3 && !carDoneFlag) {
				// System.out.println("车贷还完了：手里有[" + storage + "], 投资账户有["
				// + invest + "]");
				car -= 5000;
				carDoneFlag = true;
			}
			...
			if (i > (childyear + 1) * 365 && i < 365 * (childyear + 2)
					&& i % 30 == 0) {
				storage -= 2000;
			}
		...
		// System.out.println("allin:" + allin + ", allinvest:" + allinvest);
	}
}
